        ...
        try 
        {
            ...
            foreach(range(1,16) as $units)
            {
                ...
                foreach (['A', 'B'] as $suffix) 
                {
                    ...
                    $stmt = $conn->prepare("SELECT * FROM (SELECT * FROM tbl_event_air_flow WHERE WorkCenter = :unit ORDER BY ID DESC   ) subquery WHERE ServerDateTime >= NOW() - INTERVAL 5 HOUR ORDER BY ID ASC  ;");
            ...
            $resultList=[
                $resultLine_1A,$resultLine_1B,$resultLine_2A,$resultLine_2B,$resultLine_3A,$resultLine_3B,
