<?php
                ...
                foreach (['A', 'B'] as $suffix) 
                {
                    foreach (['Start', 'End'] as $suffix2)   //Luwa_1A_Start
                    {
                        ...
                        $stmt = $conn->prepare("SELECT * 
                                                ...
                                                WHERE WorkCenter = :unit AND ServerDateTime>= :strServerDateTime - INTERVAL 15 MINUTE
                                                ORDER BY ID DESC LIMIT 1;
                                                ");

                      //  $stmt = $conn->prepare("SELECT * FROM tbl_event_air_flow WHERE WorkCenter >= :unit AND ServerDateTime>= :strServerDateTime - INTERVAL 15 MINUTE ORDER BY ID DESC LIMIT 1;");
                        $stmt->bindParam(':unit', $unit);
                         $stmt->bindParam(':strServerDateTime', $strServerDateTime);
    ...
    //ServerDateTime
    $data_ary['Data_Ary0'] =$strServerDateTime_2;
