...
def Towerlamp(MacAdd,client,first_sensor_value,second_sensor_value):
    ...
    try:
       ...
       # print('get ToweLamp Mac ',Tower_Lamp_MAC)
        ...
        #find values count last 1 min
        Values_Count = "SELECT COUNT(Airpressure) FROM `tbl_summary_air_flow`  WHERE `LastUpdatedTime` >= DATE_SUB(%s, INTERVAL 1 MINUTE)"
        cursor = connection.cursor()
        cursor.execute(Values_Count,(server_datetime,))
        result = cursor.fetchone()
        Values_Count=result[0]
        print('Values_Count ',Values_Count)

        #find idle values count last 1 min
        Idle_Values_Count = "SELECT COUNT(Airpressure) FROM `tbl_summary_air_flow`  WHERE `LastUpdatedTime` >= DATE_SUB(%s, INTERVAL 1 MINUTE) AND Airpressure<1000;"
        cursor = connection.cursor()
        cursor.execute(Idle_Values_Count,(server_datetime,))
        result = cursor.fetchone()
        Idle_Values_Count=result[0]
        print('Idle_Values_Count ',Idle_Values_Count)
        ...
        if Values_Count==Idle_Values_Count:
            print("off")
            strPayload = ("{\"MacAdd\":\"" + Tower_Lamp_MAC + "\",\"MsgType\":\"EventTWL\",\"EventNo\":\"2\",\"Kpd_No\":\"1\",\"Data\":[0,0,0,0,0,0]}")
