...
def Timer(MacAdd):
    ...
    if MacAdd=='5C:01:3B:6B:D6:74':
        query = """SELECT ServerDateTime FROM `tbl_summary_air_pressure` WHERE UnitNumber='1A' OR  UnitNumber='1B';"""
    
    if MacAdd=='5C:01:3B:6C:25:8C':
        query = """SELECT ServerDateTime FROM `tbl_summary_air_pressure` WHERE UnitNumber='2A' OR  UnitNumber='2B';"""

    if MacAdd=='C8:F0:9E:29:93:88':
        query = """SELECT ServerDateTime FROM `tbl_summary_air_pressure` WHERE UnitNumber='3A' OR  UnitNumber='3B';"""
  ...
  #Database Connection
    ...
    #query = """SELECT ServerDateTime FROM `tbl_summary_air_pressure` WHERE UnitNumber=1 OR 2 ORDER BY ID DESC LIMIT 1;"""
    try:
    ...
    last_data_insert_time_add_5_min=last_data_insert_time+timedelta(minutes=1)
...
def insertRecord(MacAdd,first_sensor_value,second_sensor_value):
        ...
        if MacAdd=="5C:01:3B:6B:D6:74":
            ...
            cursor.execute(sql_query, (server_datetime,server_datetime,'1A',0,first_sensor_value))
            cursor.execute(sql_query2, (server_datetime,server_datetime,'1B',0,second_sensor_value))
            connection.commit()
        ...
        if MacAdd=="5C:01:3B:6C:25:8C":
            ...
            sql_query = """INSERT INTO  tbl_summary_air_pressure (ServerDateTime,LastUpdatedTime,UnitNumber,WorkCenter,Airpressure)VALUES (%s,%s,%s,%s,%s)"""
            sql_query2 = """INSERT INTO  tbl_summary_air_pressure (ServerDateTime,LastUpdatedTime,UnitNumber,WorkCenter,Airpressure)VALUES (%s,%s,%s,%s,%s)"""
            cursor.execute(sql_query, (server_datetime,server_datetime,'2A',0,first_sensor_value))
            cursor.execute(sql_query2, (server_datetime,server_datetime,'2B',0,second_sensor_value))
            connection.commit()
            #print("Mqtt Inserted to the Database")
            logging.info("Mqtt Inserted to the Database")
        ...
        if MacAdd=="C8:F0:9E:29:93:88":
            ...
            sql_query = """INSERT INTO  tbl_summary_air_pressure (ServerDateTime,LastUpdatedTime,UnitNumber,WorkCenter,Airpressure)VALUES (%s,%s,%s,%s,%s)"""
            sql_query2 = """INSERT INTO  tbl_summary_air_pressure (ServerDateTime,LastUpdatedTime,UnitNumber,WorkCenter,Airpressure)VALUES (%s,%s,%s,%s,%s)"""
            cursor.execute(sql_query, (server_datetime,server_datetime,'3A',0,first_sensor_value))
            cursor.execute(sql_query2, (server_datetime,server_datetime,'3B',0,second_sensor_value))
            connection.commit()
            #print("Mqtt Inserted to the Database")
            logging.info("Mqtt Inserted to the Database")
