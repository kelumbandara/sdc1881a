...
#topic ="sasanka99"
#topic ="Sasanka99"
topic ="sky/iot-4to20ma-module-4ch/pub/v1/svr/event/sky-iot-4to20ma-4ch/none/45236"
...
def on_message(client, userdata, msg):
    ...
    if MAC_Available_Database==1:
        ...
        if(lists==expected_json_format_4 ):
            ...
            if(msg_type=="PWR_ON"):
                ...
                PwrOnMSG(MAC,UnitID,ServerDateTime,DevicePowerOnCount)
    ...
    if MAC_Available_Database==0:
        print("MAC_Available_Database==0:")
        time.sleep(2)
        #databaseConnection()
        #MQTTConnection()
        #on_message
...
def PwrOnMSG(MAC,UnitID,ServerDateTime,DevicePowerOnCount):
    ...
    try:
        ...
        Quary_1="""SELECT DevicePowerOnCount  FROM `tblmc_devicepoweron` ORDER BY `ServerDateTime` DESC LIMIT 1"""
        cursor.execute(Quary_1)
        DB_power_on_count=cursor.fetchone()
        
        
        #Database last Reconnection On count
        Quary_2="""SELECT * FROM `tblmc_devicereconnect` ORDER BY `ServerDateTime` DESC LIMIT 1"""
        cursor.execute(Quary_1)
        DB_Reconnection_count=cursor.fetchall()
